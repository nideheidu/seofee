layui.define(['table','form','setter'],function (exports) {
    ...
    table.render({
        ...
        ,url: '/admin/keywords/shenhe?status=1' //模拟接口
        ,headers:{
            'access-token':(layui.data(setter.tableName)[request.tokenName] || '')
        }
        ,cols: [[
            ...
            ,{field:'current_ranking' ,title: '现在排名', sort:true,templet:function(d){return d.current_ranking==101 || d.current_ranking==0?">100":d.current_ranking}}
            ,{field:'create_time' ,title: '添加时间', sort:true}
            ,{field:'price' ,title: '用户单价', sort:true}
            ,{field:'agent_price' ,title: '我的成本', sort:true}
            
            ,{field: 'agent_status', title: '代理商审核状态', templet: '#agetnbuttonTpl', minWidth: 80, align: 'center'}
            ,{field: 'status', title: '平台审核状态', templet: '#buttonTpl', minWidth: 80, align: 'center'}
        ...
        if(obj.event === 'orderinfo'){
            admin.popup({
                ...
                ,yes:function (index, layero) {
                    ...
                    var web_url = $("input[name='url']").val();
                    var new_cycle = $("input[name='cycle']").val();
                    var new_price = $("input[name='price']").val();
                    var xiongzhang = $("input[name='xiongzhang']").val();

                    admin.req({
                        url: '/admin/keywords/submit'
                        ,type:"post"
                        ,data:{tid:data.id,current_ranking:new_ranking,original_rank:original_rank,web_url:web_url,cycle:new_cycle,price:new_price,xiongzhang:xiongzhang}
                        ,done: function( res ){
                            //登入成功的提示与跳转
                            layer.msg(res.msg, {
                                offset: '15px'
                ...
                ,btn2: function(index, layero){
                    var _order = $('#orderNumber').val();
                    admin.req({
                        url: '/admin/keywords/submitrefuse'
                        ,type:"post"
