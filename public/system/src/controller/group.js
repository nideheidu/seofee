...
layui.define(['table', 'form','setter'], function(exports){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'group_name', title: '会员组名'}
            ,{field: 'keyword_free', title: '首页达标扣费'}
            ,{field: 'keyword_free2', title: '次页达标扣费'}
            ,{field: 'code', title: '推荐码',align: 'center', toolbar: '#table-system-code'}
    ...
    table.on('tool(LAY-user-back-role)', function(obj){
        ...
        if(obj.event === 'del'){
            ...
            layer.confirm('确定要更新推荐码？更新后以前推荐码将失效。', function(index){
                admin.req({
                    url:'/admin/coderefresh'
                    ,type:'post'
                    ,data:data
                    ,done:function (res) {
                        layui.table.reload('LAY-user-back-role'); //重载表格
        ...
        }else if(obj.event === 'copyaddress'){
            var domain = document.domain;
            layer.alert('注册地址：</br>http://'+domain+'/customer/#/user/reg/code='+data.code);
        }
    });
