...
layui.define(['table', 'form','setter'], function(exports){
    ...
    table.render({
        ...
        ,cols: [[
            ...
            ,{field: 'keywords_count', title: '关键词数量', sort: true, align: 'center',templet: '#linkTpl'}
            ,{field: 'weburl_count', title: '客户网站', sort: true, align: 'center',templet: '#linkwebTpl'}
            ...
            ,{field: 'agent_name', title: '所属代理'}
            ,{field: 'last_login_time', title: '最近登录',minWidth: 160, align: 'center'}
            ,{field: 'create_time', title: '注册时间', sort: true, minWidth: 100, align: 'center'}
            ,{field: 'is_domain', title: '完全匹配', minWidth: 100, align: 'center'}
    ...
    table.on('tool(LAY-app-customer-list)', function(obj){
        ...
        if(obj.event === 'editBalance'){
            ...
            // console.log(data);
            cookie.set("seoid",data.id,1);
            localStorage.setItem("seoid",data.id);
            // alert(localStorage.getItem("seoid"));
            // alert(window.location.host+"/admincustomer/#/user/login");
            window.open("http://"+window.location.host+"/admincustomer/#/user/login")
            // alert(window.location.host);
            //window.open("http://www.100sucai.com/"); 
        }else if(obj.event === 'tasklist'){
            
            window.open("http://"+window.location.host+"/platform/#/seo/task/all/uid="+data.id)
