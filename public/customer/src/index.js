...
layui.extend({
    ...
    var setter = layui.setter
        ...
        ,entryPage = function(fn){
            ...
            //独立页面
            if(isIndPage || pathURL === '/user/login' || pathURL === '/user/reg'){ //此处单独判断登入页，是为了兼容旧版（即未在 config.js 配置 indPage 的情况）
        ...
        ,$ = layui.$, $win = $(window);
        if(admin.correctRouter(router.path.join('/')) != '/user/reg')
        {
            admin.req({
            url:'/v1/init'
                ,done:function (res) {
                    layui.setter.name = res.data.site_name;
